...
in
{
  ...
  config = {
    assertions = with noxa.lib.ansi; [
      {
        assertion = ((!config.age.rekey.initialRollout) && length (attrNames config.age.secrets) > 0) -> (config.age.rekey.storageMode == "local" -> filesystem.pathIsDirectory config.age.rekey.localStorageDir);
